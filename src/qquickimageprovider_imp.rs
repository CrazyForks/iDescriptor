...
pub trait AddImageProvider<P: QQuickImageProvider> {
    ...
    fn add_image_provider(&mut self, provider_id: &str, provider: QObjectBox<P>);
}
...
impl<P: QQuickImageProvider> AddImageProvider<P> for QmlEngine {
    fn add_image_provider(&mut self, provider_id: &str, provider: QObjectBox<P>) {
        ...
        let provider_ptr = provider.pinned().get_or_create_cpp_object();
        cpp!(unsafe [
        ...
        // FIXME: this or Box::leak(provider); ?
...
cpp! {{
    #include "src/qmetaobject_rust.hpp"
    #include <QtQuick/QQuickImageProvider>

    #if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
    class QPixmapProvider : public QQuickImageProvider
    {
        public:
            QPixmapProvider(): QQuickImageProvider(ImageType::Pixmap) {}
    };
    #else
    ...
    {
        public:
            QImageProvider(): QObject(), QQuickImageProvider(ImageType::Image) {}
    };
    #endif
    ...
    struct Rust_QImageProvider : public RustObject<QImageProvider>
    {
        QImage requestImage(const QString &id, QSize *size, const QSize &requested_size) override
        {
            return rust!(Rust_QImageProvider_request_image [
                rust_object: QObjectPinned<dyn QQuickImageProvider> as "TraitObject",
                id: &QString as "const QString &",
                size: *mut QSize as "QSize *",
                requested_size: &QSize as "const QSize &"
            ] -> QImage as "QImage" {
